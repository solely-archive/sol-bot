...
client.Dispatcher.on('MESSAGE_CREATE', (e) => {
  if (e.message.content == prefix + 'ping') {
    e.message.channel.sendMessage('ping').then((sentMessage) => {
  ...
  if (e.message.content.startsWith(prefix + 'rate ')) {
    let ratething = e.message.content.substring(6, 2000);
    let randomvalue1 = Math.floor(Math.random() * 11);
    let randomvalue2 = Math.round(randomvalue1);
  ...
  if (e.message.content.startsWith(prefix + 'cat')) {
    let randomvalue1 = Math.floor(Math.random() * 999);
    let randomvalue2 = Math.round(randomvalue1);
  ...
  if (e.message.content == prefix + 'userinfo') {
    e.message.channel.sendMessage(`User info about: ${e.message.author.username}\n
